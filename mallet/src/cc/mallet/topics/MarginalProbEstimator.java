...
public class MarginalProbEstimator implements Serializable {
	...
	public BigDecimal evaluateLeftToRight (InstanceList testing, int numParticles, boolean usingResampling,
									   PrintStream docProbabilityStream) {
		/* Iman */
		double dScore = 0.0;
		int iDocLength = 0;
		BigInteger iDocTotalLength = BigInteger.valueOf(0);
		...
		BigDecimal totalLogLikelihood2 = BigDecimal.valueOf(0.0);
		for (Instance instance : testing) {
			...
			if (docProbabilityStream != null) {
				...
				/* Iman */
				iDocLength = ((FeatureSequence)instance.getData()).size();
				
				iDocTotalLength = iDocTotalLength.add(BigInteger.valueOf(iDocLength));
				/*System.out.println(instance.getName() + " -> " +  docLogLikelihood + " -> " 
						+ iDocLength);*/
				if(iDocLength != 0)
					dScore = docLogLikelihood/iDocLength;
				else
					dScore = 0;
				docProbabilityStream.println(docLogLikelihood + "\t" + dScore);
			}
			totalLogLikelihood += docLogLikelihood;
			/* Iman */
			totalLogLikelihood2 = totalLogLikelihood2.add(BigDecimal.valueOf(dScore/testing.size()));
		}
		System.out.println("Total log likelihood = " + totalLogLikelihood
				+ ", Total num of words = " + iDocTotalLength.toString());
		return totalLogLikelihood2;
	}
