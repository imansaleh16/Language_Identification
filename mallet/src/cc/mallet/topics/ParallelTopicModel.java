...
public class ParallelTopicModel implements Serializable {
	...
	public void optimizeAlpha(WorkerRunnable[] runnables) {
		...
		if (usingSymmetricAlpha) {
			System.out.println("Symmetric alpha ...");
			...
			System.out.println("Asymmetric alpha ...");
			try {
	...
	public void estimate () throws IOException {
		...
		if (numThreads > 1) {
			...
			System.out.println("Initialize alpha statistics with " + docLengthCounts.length);
	...
	public void printDocumentTopics (File file) throws IOException {
		...
		/* Iman */
		/*PrintWriter out2 = new PrintWriter (new FileWriter (file + "_2") );
		printTopicDocuments(out2, data.size());
		out2.close();*/
	}
	...
	public void printDocumentTopicsLabeledLDA (PrintWriter out) {
		printDocumentTopics (out, 0.0, 5);
	}
	...
	public ArrayList<TreeSet<IDSorter>> getTopicDocuments_2(double smoothing) {
		ArrayList<TreeSet<IDSorter>> topicSortedDocuments = new ArrayList<TreeSet<IDSorter>>(numTopics);
		double dTmp;
		// Initialize the tree sets
		for (int topic = 0; topic < numTopics; topic++) {
		...
		try {
			BufferedWriter bwTopicDocs = new BufferedWriter(new FileWriter(new File("topicDocsTmp")));
			...
			for (int doc = 0; doc < data.size(); doc++) {
				...
				for (int topic = 0; topic < numTopics; topic++) {
					...
					dTmp = (topicCounts[topic] + smoothing) / (topics.length + numTopics * smoothing);
					//bwTopicDocs.write(topic + "\t" + doc + "\t" + dTmp + "\n");
					//bwTopicDocs.flush();
			...
			bwTopicDocs.close();
			Runtime.getRuntime().exec("sort -nk1,1 -nk3,3 topicDocsTmp > topicDocsTmp.sorted");
		} catch (IOException e) {
