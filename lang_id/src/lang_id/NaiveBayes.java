...
public class NaiveBayes {
	...
	public static void main(String[] args) throws IOException{
		...
		if(args[0].compareTo("train")==0)
		{
			...
			nb.createTrainCounts(S_TRAIN, S_TRAIN_LABELS);
		}
		else if(args[0].compareTo("test")==0)
		{
			hmssLangNames = Utils.loadLabels();
			nb.test(S_TEST, S_TEST_LABELS);
		}
		else{
			hmssLangNames = Utils.loadLabels();
	...
	/*
	 * Generate Naive Bayes classifier probabilities:
	 * p(lang_i) = num_of_docs(lang_i)/total number of documents
	 * p(ngram_i|lang_j) = [count(ngram_i,lang_j) + 1]/[total num of ngrams of lang_j + vocabulary size]
	 *  
	 */
	public void createTrainCounts(String sTrainFile, String sLabelsFile) throws IOException
	{
		...
		while((sLine=brTrain.readLine())!=null)
		{
			...
			for(int i=0; i<sLine.length()-IGRAM + 1; i++)
			{
				sGram = sLine.substring(i, i + IGRAM).toLowerCase();
	...
	/*
	 * Writes vocabulary size to a file to be used in testing. 
	 */
	...
	/*
	 * Generate language probabilities for test documents.
	 */
	public void test(String sTestFile, String sTestFileLabels) throws IOException
	{
		...
		BufferedWriter bw = new BufferedWriter(new FileWriter("naive_answer"));
		...
		while((sLine=brTest.readLine())!=null)
		{
			...
			hmsdLangProbTest = calculateLangProbs(sLine);
	...
	public void getDocLanguage(String sDocPath) throws IOException
	{
		...
		hmsdLangProbTest = calculateLangProbs(sDoc);
		sAnswer = Utils.getMaxProb(hmsdLangProbTest);
		System.out.println(hmssLangNames.get(sAnswer));
	}
	
	/*
	 * Calculate languages probabilities for a document.
	 */
	private HashMap<String, Double> calculateLangProbs (String sDoc){
		String sGram = "";
		int iNGramCount;
		double dNGramProb;
		...
		// Create ngrams and store them for each language.
		for(int i=0; i<sDoc.length()-IGRAM+1; i++)
		{
			sGram = sDoc.substring(i, i+IGRAM).toLowerCase();
